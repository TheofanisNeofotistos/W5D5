...
def bad_years
  ...
  Movie.group(:yr).where.not(:yr => Movie.select(:yr).where("score > 8")).pluck(:yr)
...
def vanity_projects
  ...
  Movie.joins(:actors).select(:id,:title,:name).where("actors.id = movies.director_id").where("ord = 1" )
...
def most_supportive
  ...
  Actor.joins(:movies).select(:id, :name, 'COUNT(*) AS roles').where("castings.ord > 1").group(:id).order("roles DESC").limit(2)
end